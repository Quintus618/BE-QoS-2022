...
public class bandwidthbroker{
    ...
    private String portSIP="5060";//attention tout ça est statique et n supporte pas de modifications de la topographie du réseau
    ...
    public bandwidthbroker(){
    	...
    	String mangle_init= "iptables -t mangle -A FORWARD -p udp --dport "+portSIP+" -j DSCP --set-dscp-class EF;iptables -t mangle -A OUTPUT -p udp --dport "+portSIP+" -j DSCP --set-dscp-class EF;";
        ...
        try {
 			askSSH(this.CE_A, mangle_init);
 	        askSSH(this.CE_B, mangle_init);//TODO décommenter
 		} catch (Exception e) {
 			e.printStackTrace();
    ...
    public synchronized int addRessourcesConnexions(InetAddress source, InetAddress receiver, int portSource, int portRecei, double demand){
        ...
        if(demand <=0){
	        ...
	        System.out.println("La connexion va être ajoutée...");
	        }
	    }
    ...
    private void qdisc_EF_branch_update(boolean creatrue_removalse, int debit_asked, InetAddress ipsource, int portsource, int indice) throws Exception{
       ...
       if (creatrue_removalse){
            ...
            update_tc = update_tc+";tc filter add dev eth0 parent 1:1 protocol ip prio 1 u32 match ip src "+addr_src+"/32 match ip dport "+Integer.toString(portsource)+" 0xffff flowid 1:"+indc;
       
        }else{//note: il est obligatoire de del tous les filters et classes filles avant de supprimer une classe
            update_tc = "tc filter del dev eth0 parent 1:1 protocol ip prio 1 u32 match ip src "+addr_src+"/32 match ip dport "+Integer.toString(portsource)+" 0xffff flowid 1:"+indc;
